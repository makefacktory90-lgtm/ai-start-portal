...
| 9 | Granola / Obsidian / Whisper route | pilot / по security policy | ведущая, методолог, owners | запись созвонов, notes, transcript, база знаний |
| 10 | Транскрипция | 1 согласованный маршрут | редакторы / видео | transcript + timestamps для текстового и видео-пайплайна |
...
| Granola | ведущая, методолог, owners | meeting capture, notes, action items, follow-up memory | пилотно, только после проверки политики безопасности |
| Obsidian | ведущая, методолог, owners / advanced | local-first markdown vault, role OS, hub notes, backlinks | дать как optional knowledge base route |
| Whisper → Obsidian | technical helper / owners | transcription route into markdown notes and hub artifacts | использовать для записей и созвонов, где разрешена обработка |
...
## 5.3. Meeting capture / knowledge base layer: Granola, Obsidian, Whisper

Это слой для того, чтобы созвоны, разборы и решения не исчезали в личной памяти.

| Инструмент / маршрут | Для чего | Как показываем |
|---|---|---|
| Granola | AI-notes по встречам: raw notes + transcript + enhanced summary + action items | быстрый capture layer для ведущей / owners |
| Obsidian | local-first markdown vault, backlinks, role OS, заметки курса | личная и методологическая база знаний |
| Whisper → Obsidian | запись → транскрипт → структурная markdown-заметка → hub update | маршрут для записей, если нужна контролируемая обработка |

Ограничение:

```text
Этот слой нельзя включать без security / privacy проверки.
Для корпоративных созвонов важно заранее решить:
что можно записывать, где хранить transcript, кому доступны ссылки и можно ли загружать внутренние материалы во внешний сервис.
```

Рекомендация:

```text
Показать этот слой в курсе как meeting-to-knowledge workflow.
Не покупать всем на старте.
Дать ведущей, методологу и owners как pilot / optional route.
```
...
| Meeting capture / knowledge base | Granola, Obsidian, Whisper route | показываем как созвон → transcript → summary → hub update |
